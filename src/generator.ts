...
export class OpenAPITypeScriptGenerator {
  ...
  generate(): string {
    ...
    if (this.options.header !== false) {
      if (this.options.headerComment) {
        parts.push(this.options.headerComment)
      }
      else {
        parts.push('/**')
    ...
    if (this.options.inject) {
      parts.push(this.options.inject)
      parts.push('')
    }
    ...
    // Apply post-transform if provided
    if (this.options.postTransform) {
      result = this.options.postTransform(result)
    }
  ...
  private generateObjectType(schema: SchemaObject, depth: number): string {
    ...
    for (const [propName, propSchema] of Object.entries(schema.properties)) {
      ...
      const readonly = this.options.immutable ? 'readonly ' : ''

      // Apply transform if provided
      const transformedSchema = this.options.transform ? this.options.transform(propSchema as SchemaObject) : propSchema
      const propType = this.generateSchemaType(transformedSchema, depth + 1)

      // Build JSDoc comment
      const jsdocLines: string[] = []
      if (this.options.includeDescriptions !== false && 'description' in transformedSchema && transformedSchema.description) {
        jsdocLines.push(transformedSchema.description)
      }
      if (this.options.includeExamples && 'example' in transformedSchema && transformedSchema.example !== undefined) {
        jsdocLines.push(`@example ${JSON.stringify(transformedSchema.example)}`)
      }
      if ('deprecated' in transformedSchema && transformedSchema.deprecated) {
        jsdocLines.push('@deprecated')
      }

      // Add JSDoc comment if we have content
      if (jsdocLines.length > 0) {
        if (jsdocLines.length === 1) {
          parts.push(`${indent}/** ${jsdocLines[0]} */`)
        }
        else {
          parts.push(`${indent}/**`)
      ...
      parts.push(`${indent}${readonly}"${propName}"${optional}: ${propType}`)
    }
...
export async function generateTypes(options: GeneratorOptions): Promise<void> {
  ...
  if (!options.silent) {
    console.log(`✅ Generated TypeScript definitions: ${output}`)
  }
}
